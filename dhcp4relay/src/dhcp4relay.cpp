...
bool addr_is_primary(const std::string &ifname, const struct in_addr *addr) {
    ...
    for (const auto &table_name : tables) {
        auto match_pattern = std::string("*") + table_name + "|" + ifname + "|*";
 ...
 *                      the socket will be created when an interface update event arrives.
 *
 ...
 *
 * @return              int (0 on success, -1 on failure or deferred)
 */
int prepare_interface_sockets(relay_config &config) {
    ...
    if (!bind_client_addr) {
        SWSS_LOG_NOTICE("[DHCPV4_RELAY] No IPv4 address on interface %s, deferring socket creation", config.interface.c_str());
        return -1;
    }
    ...
    if (setsockopt(client_sock, SOL_SOCKET, SO_BINDTODEVICE, config.interface.c_str(),
        ...
        SWSS_LOG_ERROR("[DHCPV4_RELAY] failed to bind client_sock to interface %s, error: %s",
               config.interface.c_str(), strerror(errno));
        close(client_sock);
        return -1;
    }
...
void encode_relay_option(pcpp::DhcpLayer *dhcp_pkt, relay_config *config) {
    ...
    if (!config->circuit_id_format.empty()) {
    ...
    if (!(dhcp_pkt->getDhcpHeader()->gatewayIpAddress)) {
        ...
        if (!(dhcp_pkt->getDhcpHeader()->gatewayIpAddress)) {
            SWSS_LOG_ERROR("[DHCPV4_RELAY] No IPv4 address configured on %s,"
                   " dropping DHCP packet. Configure an IPv4 address on the"
                   " interface for relay to function", config.interface.c_str());
...
void to_client(pcpp::DhcpLayer *dhcp_pkt, std::unordered_map<std::string, relay_config> *interfaces,
    ...
    if (options_ptr != NULL) {
        ...
        if (relay_interface.length() > 0) {
            ...
            if (config_itr == interfaces->end()) {
                SWSS_LOG_INFO("[DHCPV4_RELAY] Interface config not found for the circuit"
    ...
    if (config_itr == interfaces->end()) {
        ...
        if (config_itr == interfaces->end()) {
            SWSS_LOG_ERROR("[DHCPV4_RELAY] Config not found for interface %s", intf_name.c_str());
    ...
    if (config.client_sock <= 0) {
        SWSS_LOG_WARN("[DHCPV4_RELAY] Dropping server reply for %s: interface socket not ready (no IPv4 address)",
               config.interface.c_str());
        dhcp_cntr_table.increment_counter(config.interface, "TX", DHCPv4_MESSAGE_TYPE_DROP);
        return;
    }
    ...
    if (send_udp(config.client_sock, (uint8_t *)dhcp_pkt->getDhcpHeader(), target_addr, dhcp_pkt->getHeaderLen(), ip_zero, false, pad)) {
        ...
        SWSS_LOG_WARN("[DHCPV4_RELAY] Failed to send server reply to client on %s", config.interface.c_str());
        dhcp_cntr_table.increment_counter(config.interface, "TX", DHCPv4_MESSAGE_TYPE_DROP);
    }
}
...
void update_port_interface_mapping(std::string port, std::string relay_interface, bool is_add) {
    if (is_add) {
        ...
        SWSS_LOG_INFO("[DHCPV4_RELAY] Add <%s, %s> into port interface map", port.c_str(), relay_interface.c_str());
    } else {
        interface_map.erase(port);
        dhcp_cntr_table.remove_interface(relay_interface);
        SWSS_LOG_INFO("[DHCPV4_RELAY] Remove <%s, %s> from port interface map", port.c_str(), relay_interface.c_str());
    }
}
...
void update_interface_mapping(std::string interface_name, bool is_add) {
    ...
    if (!keys.empty()) {
        ...
        SWSS_LOG_INFO("[DHCPV4_RELAY] %s <%s, %s> %s interface map (physical port)",
        ...
        if (dhcp_pkt->getDhcpHeader()->opCode == BOOTPREQUEST) {
            ...
            if (config_itr == interfaces->end()) {
                SWSS_LOG_INFO("[DHCPV4_RELAY] Relay config not found for %s (interface %s, vlan_id %d)",
                       relay_intf_str.c_str(), intf.c_str(), vlan_id);
                dhcp_cntr_table.increment_counter(relay_intf_str, "RX", DHCPv4_MESSAGE_TYPE_DROP);
                continue;
            }
            auto config = config_itr->second;
...
/*
 * Apply one CONFIG_DB-derived event to the interfaces map. Used both from
...
static void apply_config_event(const event_config &received_event,
	   ...
	   (received_event.type == DHCPv4_SERVER_RELAY_CONFIG_UPDATE))	{
            ...
            if (relay_msg) {
                SWSS_LOG_INFO("[DHCPV4_RELAY] Processing config update: Interface %s", relay_msg->interface.c_str());
                ...
                if (relay_msg->is_add) {
                    if (interfaces->find(relay_msg->interface) == interfaces->end()) {
                        ...
                        if (prepare_interface_sockets((*interfaces)[relay_msg->interface]) == -1) {
                            SWSS_LOG_ERROR("[DHCPV4_RELAY] Failed to create interface listen socket");
                            return;
                        }
                    ...
                    /* Keep struct .interface in sync with DB (repairs ghosts from INTERFACE_UPDATE ordering). */
                    (*interfaces)[relay_msg->interface].interface = relay_msg->interface;
                    ...
                    if (interfaces->find(relay_msg->interface) != interfaces->end()) {
                        ...
                        SWSS_LOG_INFO("[DHCPV4_RELAY] Deleted interface %s from configuration", relay_msg->interface.c_str());
                        update_interface_mapping(relay_msg->interface, false);
                    } else {
                        SWSS_LOG_WARN("[DHCPV4_RELAY] Attempted to delete non-existent interface %s", relay_msg->interface.c_str());
                    }
                }
                delete relay_msg;
            }
            ...
            if (relay_msg) {
                if (interfaces->find(relay_msg->interface) == interfaces->end()) {
                    SWSS_LOG_WARN("[DHCPV4_RELAY] Ignoring INTERFACE_UPDATE for unknown interface %s",
                                  relay_msg->interface.c_str());
                    delete relay_msg;
                    return;
                }
                SWSS_LOG_INFO("[DHCPV4_RELAY] Updating source interface for interface %s", relay_msg->interface.c_str());
                if (relay_msg->is_add) {
                    interfaces->at(relay_msg->interface).src_intf_sel_addr = relay_msg->src_intf_sel_addr;
                } else {
                    memset(&interfaces->at(relay_msg->interface).src_intf_sel_addr, 0, sizeof(sockaddr_in));
                }
                delete relay_msg;
            }
               ...
               if (msg) {
                   ...
                   if ((*interfaces)[msg->vlan].client_sock > 0) {
                       ...
                       (*interfaces)[msg->vlan].client_sock = 0;
                   }
                   ...
                   if (prepare_interface_sockets((*interfaces)[msg->vlan]) == -1) {
                       SWSS_LOG_NOTICE("[DHCPV4_RELAY] Interface socket not ready for %s, will create when IPv4 is assigned",
                              msg->vlan.c_str());
                   }
            ...
            if (relay_msg) {
                if (relay_msg->is_add) {
                    SWSS_LOG_INFO("[DHCPV4_RELAY][DualTor] Adding link-selection and source-interface as Loopback0 for existing interfaces");
                } else {
                    SWSS_LOG_INFO("[DHCPV4_RELAY][DualTor] Deleting/Restoring link-selection and source-interface configs for existing interfaces");
                }
                ...
                for (auto& intf : *interfaces) {
                    if (relay_msg->is_add) {
                        ...
                        v4_relay_intf_tbl->hget(intf.second.interface, LINK_SELECTION_FIELD, value);
                        ...
                        v4_relay_intf_tbl->hget(intf.second.interface, SOURCE_INTERFACE_FIELD, value);
...
void config_event_callback(evutil_socket_t fd, short event, void *arg) {
    ...
    if (bytes_read == sizeof(received_event)) {
        ...
        apply_config_event(received_event, interfaces);
    } else {
...
void loop_relay(std::unordered_map<std::string, relay_config> &interfaces) {
     ...
     */
    {
        ...
        while (poll(&pfd, 1, -1) > 0 &&
            ...
            apply_config_event(ev, &interfaces);
        }
    }
