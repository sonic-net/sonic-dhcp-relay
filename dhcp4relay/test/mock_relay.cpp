...
MOCK_GLOBAL_FUNC1(if_nametoindex, unsigned int(const char *));
...
TEST(DHCPRelayTest, passthrough_frame_rejects_invalid_args) {
    ...
    EXPECT_GLOBAL_CALL(if_nametoindex, if_nametoindex(_)).Times(0);
    EXPECT_GLOBAL_CALL(sendmsg, sendmsg(_, _, _)).Times(0);
    EXPECT_FALSE(passthrough_frame(-1, "Vlan100", 5, frame, sizeof(frame)));
    EXPECT_FALSE(passthrough_frame(1, "", 5, frame, sizeof(frame)));
    EXPECT_FALSE(passthrough_frame(1, "Vlan100", 5, nullptr, sizeof(frame)));
    EXPECT_FALSE(passthrough_frame(1, "Vlan100", 5, frame, 0));
}

TEST(DHCPRelayTest, passthrough_frame_unknown_vlan) {
    uint8_t frame[64] = {0};

    EXPECT_GLOBAL_CALL(if_nametoindex, if_nametoindex(StrEq("Vlan999"))).WillOnce(Return(0));
    EXPECT_GLOBAL_CALL(sendmsg, sendmsg(_, _, _)).Times(0);
    EXPECT_FALSE(passthrough_frame(1, "Vlan999", 5, frame, sizeof(frame)));
}

TEST(DHCPRelayTest, passthrough_frame_success) {
    uint8_t frame[64] = {0};

    EXPECT_GLOBAL_CALL(if_nametoindex, if_nametoindex(StrEq("Vlan100"))).WillOnce(Return(42));
    EXPECT_GLOBAL_CALL(sendmsg, sendmsg(_, _, _))
        .WillOnce([](int, const struct msghdr *msg, int) {
            ...
            EXPECT_EQ(dest->sll_ifindex, 42);
            return 64;
        });
    EXPECT_TRUE(passthrough_frame(1, "Vlan100", 5, frame, sizeof(frame)));
}
...
TEST(DHCPRelayTest, passthrough_frame_send_failure) {
    ...
    EXPECT_FALSE(passthrough_frame(1, "Vlan100", 3, frame, sizeof(frame)));
}
